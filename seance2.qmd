...
::: {.callout-note appearance="simple" icon="false" collapse="true"}
## 📝 Tâche 19 : Répliquez l'analyse sur l'échantillon en excluant les *outliers*.

Pour ce faire, le plus simple est de reprendre le code précédent et d'ajouter un filtre, via la fonction **filter()**, sur la base traiter en retenant les limites fixées pour désigner les observations hors *outliers*. Celles-ci sont les mêmes que précédemment : moyenne des rendements moins trois écart-types des rendements et moyenne des rendements plus trois écart-types des rendements.
...
On notera que l'exclusion des *outliers* ne change rien à l'inférence. Les variables associées aux phénomènes testés demeurent statistiquement non significatifs. On peut néanmoins noter l'augmentation de la constante suite à l'exclusion des *outliers.* Cela montre simplement que ceux-ci étaient plus fréquemment des rendements négatifs importants.

Avant de conclusion la tache, voyons la multicolinéarité dans régression complète. Encore un fois utilisons la fonction **vif()** du package *car*.

```{r}
car::vif(reg9)
```

Le diagnostique reste le même qui précédemment. Les vif reste très faible (proche de 1). Il n'y a pas de problèmes de multicolinéarité. Cela n'est pas très surprenant.
:::
